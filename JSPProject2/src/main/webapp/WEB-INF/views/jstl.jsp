...
<body>
    ...
    <pre>
        JSP 파일 최상단에 추가하고자 하는 JSTL 태그 taglib를 추가
    </pre>
	...
	<!-- JSTL 작성법 -->
    <c:set var="num2" value="20"/> <!-- jsp가 서블릿으로 변환될 때 위 스크립틀릿 작성법으로 변환됨 -->
    ${num1 } / ${num2 }
    
    <!-- request scope에 num2 세팅-->
    <c:set var="num2" value="300" scope="request"/>

	<h4>${num2 }</h4> <!-- 20 : scope 우선순위 --> 
	<h4>${requestScope.num2 }</h4> <!-- 300 : 우선순위 무시 스코피 명시 -->
	...
	<p>
		...
		<%-- 특정 범위 삭제 / 모든 범위 삭제 --%>
		<%-- <c:remove var="test"/> --%>
		<c:remove var="test" scope="request"/> <!-- request 범위에 있는 test변수만 삭제 -->
	...
	<%--
		...
		*속성값을 감싸고 있는 따옴표종류와 다른 종류로 리터럴 표시*
	--%>
	
	<%-- 단독 if문인 경우의 단점 : 
			1) else 상황에 대한 if문을 별도 작성 필요
			2) 효율성 감소 문제(모든 if문 검사 진행) 
	--%>
	...
	<hr>5 - 반복문( c:forEach 태그 )<hr>
	
	<pre>
		- 일반 for + 추가 기능
		
		- 속성
		1) var   : 현재 반복 ㅚㅅ수에 해당하는 변수 (int i)
		2) begin : 반복 시 var의 시작 값
		3) end   : 반복이 종료될 var 값
