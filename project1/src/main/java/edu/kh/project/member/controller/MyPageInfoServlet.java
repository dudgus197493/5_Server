...
public class MyPageInfoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 파라미터 얻어오기
		String memberNuckname = req.getParameter("memberNickname");
		String memberTel = req.getParameter("memberTel");
		
		String[] arr = req.getParameterValues("memberAddress");
		
		String memberAddress = "";
		if(!arr[0].equals("") && !arr[1].equals("")) { // 주소가 입력되었을 때
		...
		member.setMemberNickname(memberNuckname);
		...
		try {
			...
			if (result > 0) { // 수정 성공
				...
				// (데이터 동기화)
				loginMember.setMemberNickname(memberNuckname);
			...
			String errorMessage = "회원 정보 변경 중 문제가 발생했습니다.";
